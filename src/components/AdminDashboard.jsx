...
import AdminModalAddAdmin from "./AdminModalAddAdmin";
...
function AdminDashboard() {
  ...
  useEffect(() => {
    getAllUsers();
  }, [showDelete, showDeleteCer, showModalEditAccount, showModalEditCert, showAddAdmin]);
    ...
    <>
      ...
      <Container className="my-5">
        <div className="d-flex flex-row-reverse my-3">
          ...
          <button
            type="button"
            style={{ backgroundColor: "#22a7e0", border: "none", borderRadius: "10px", color: "#ffffff", fontSize: "rem" }}
        ...
        <h1 className="mb-1 text-center">Lista Utenti Attivi </h1>
        ...
        {users.length === 0 ? (
          ...
          <Row>
            {users
              .filter((user) => user.name.toLowerCase().includes(searchQuery) || user.surname.toLowerCase().includes(searchQuery))
              .map((user) => (
            ...
            {setShowAddAdmin && <AdminModalAddAdmin show={showAddAdmin} onHide={() => setShowAddAdmin(false)} />}
          </Row>
        )}
      </Container>
    </>
